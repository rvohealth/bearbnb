...
export default async function createPlace(attrs: UpdateableProperties<Place> = {}) {
  ...
  // run `Restart TS Server` (command-shift-p and type "Restart TS Server" in VSCode), and if that doesn't work
  // restart your editor. Editors sometime have trouble updating to reflect changes to files that are not open
  // in the editor, as when Dream or Psychic update the type files in api/src/types/
